...
trait SocketIOActions extends Results with BodyParsers {
  ...
  def initSession = Action {
    ...
    Ok(sessionId + ":" + t + ":" + t + ":websocket,xhr-polling")
  }
  ...
  (Iteratee[String, _], Enumerator[String]) = {
  ...
  def handleConnectionFailure(error: String): (Iteratee[String, _], Enumerator[String]) = {
    // Connection error
  ...
  def xhrHandler(sessionId: String) = Action.async {
    implicit request =>
...
class WSActor(channel: Channel[String], processMessage: (String, Packet) => Unit, wsMap: collection.mutable.Map[String, ActorRef], wsRevMap: collection.mutable.Map[ActorRef, String], clientTimeout: Timeout) extends Actor {
