    ...
    def invokeSecuredBlock[A](ajaxCall: Boolean, authorize: Option[Authorization], request: Request[A],
        ...
        if (authorize.isEmpty || authorize.get.isAuthorized(user)) {
          ...
          Future.successful {
            if (ajaxCall) {
              ...
              Redirect(RoutesHelper.notAuthorized.absoluteURL(UserProvider.sslEnabled))
            }
          }
        }
      }
      ...
      result.getOrElse({
        if (Logger.isDebugEnabled) {
          Logger.debug("[Secure] anonymous user trying to access : '%s'".format(request.uri))
        }
        val response = if (ajaxCall) {
          ajaxCallNotAuthenticated(request)
        } else {
          Redirect(RoutesHelper.login().absoluteURL(UserProvider.sslEnabled))
            .flashing("error" -> Messages("loginRequired"))
            .withSession(session + (Secure.OriginalUrlKey -> request.uri)
            )
        }
...
object Secure {
   ...
   * @return
   */
  def currentUser[A](implicit request: RequestHeader): Option[User] = {
    request match {
   ...
   */
  def serviceInfoFor(user: User): Option[ServiceInfo] = {
    Registry.providers.get(user.providername) match {
      case _ => None
    }
  }
  ...
  def withRefererAsOriginalUrl[A](result: Result)(implicit request: Request[A]): Result = {
    request.session.get(OriginalUrlKey) match {
      ...
      case Some(_) => result
      case None => {
        request.headers.get(HeaderNames.REFERER).map {
          referer =>
  ...
  val enableRefererAsOriginalUrl = {
    ...
    Play.current.configuration.getBoolean("enableRefererAsOriginalUrl").getOrElse(false)
  }
}
