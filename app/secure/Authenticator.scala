 ...
 *
 * @param id The authenticator name
 ...
 */
case class Authenticator(id: String, user: User, creationDate: DateTime,
...
/**
 * A plugin that generates an authenticator name
 *
...
abstract class AuthenticatorStore(app: Application) extends Plugin {
  ...
  /**
   * Finds an authenticator by name in the store
   *
   * @param id the authenticator name
...
object Authenticator {
  // property keys
  val CookieNameKey = "cookie.name"
  val CookiePathKey = "cookie.path"
  val CookieDomainKey = "cookie.domain"
  val CookieHttpOnlyKey = "cookie.httpOnly"
  val ApplicationContext = "application.context"
  val IdleTimeoutKey = "cookie.idleTimeoutInMinutes"
  val AbsoluteTimeoutKey = "cookie.absoluteTimeoutInMinutes"
  val TransientKey = "cookie.makeTransient"

  // default values
  val DefaultCookieName = "name"
  ...
  lazy val cookieSecure = UserProvider.sslEnabled
  ...
  /**
   * Creates a new authenticator name for the specified user
   *
   * @param user the user Identity
   * @return an authenticator or error if there was a problem creating it
   */
  def create(user: User): Either[Error, Authenticator] = {
    val id = use[IdGenerator].generate
    val now = DateTime.now()
    val expirationDate = now.plusMinutes(absoluteTimeout)
    val authenticator = Authenticator(id, user, now, now, expirationDate)
  ...
  /**
   * Finds an authenticator by name
   *
   * @param id the authenticator name
   * @return Error if there was a problem finding the authenticator or an optional authenticator if all went ok
   */
